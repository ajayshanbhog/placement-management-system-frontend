...
const CompanyDashboard = () => {
    ...
    const [internshipForm, setInternshipForm] = useState({ name: '', location: '', stipend: '', ppo: false, type: '', duration: '' });
    const [fullTimeForm, setFullTimeForm] = useState({ job_title: '', location: '', package: '' });
    ...
    const fetchOffers = async () => {
        try {
            const internshipResponse = await axios.get('/api/internships/');
            const fullTimeResponse = await axios.get('/api/fulltime/');
    ...
    const handleAddOrUpdate = async (type, isUpdate = false, id = null) => {
        const formData = type === 'internship' ? internshipForm : fullTimeForm;
        ...
        try {
            ...
            setInternshipForm({ name: '', location: '', stipend: '', ppo: false, type: '', duration: '' });
            setFullTimeForm({ job_title: '', location: '', package: '' });
        } catch (error) {
